...
export function LiveReadinessWorkspace({ isAdmin, onMessage }: Props) {
  ...
  return <section>
    ...
    <article className="glass-danger mt-5 flex gap-3 rounded-md p-4 text-sm text-slate-200">
      ...
      <p><strong className="text-white">Live execution hard lock active.</strong> The broker adapter, live risk engine, reconciliation and activation path now all exist and are enforced. What holds execution shut is <span className="font-mono">LIVE_TRADING_ENABLED</span>: the application refuses to start with it set, unconditionally, and nothing in this screen can change that. Recording a review here changes no trading control.</p>
    </article>
